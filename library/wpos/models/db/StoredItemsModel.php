...
class StoredItemsModel extends DbConfig
{
    ...
    {
        $sql          = "INSERT INTO stored_items (`data`, `supplierid`, `categoryid`, `code`, `name`, `price`, `wholesale_price`) VALUES (:data, :supplierid, :categoryid, :code, :name, :price, :wholesale_price);";
        $placeholders = [":data"=>json_encode($data),":supplierid"=>$data->supplierid, ":categoryid"=>$data->categoryid, ":code"=>$data->code, ":name"=>$data->name, ":price"=>$data->price, ":wholesale_price"=>$data->wholesale_price];
    ...
    public function edit($id, $data){
        ...
        $sql = "UPDATE stored_items SET data= :data, supplierid= :supplierid, categoryid= :categoryid, code= :code, name= :name, price= :price, wholesale_price= :wholesale_price WHERE id= :id;";
        $placeholders = [":id"=>$id, ":data"=>json_encode($data), ":supplierid"=>$data->supplierid, ":categoryid"=>$data->categoryid, ":code"=>$data->code, ":name"=>$data->name, ":price"=>$data->price, ":wholesale_price" => $data->wholesale_price];
